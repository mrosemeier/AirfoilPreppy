...
class Airfoil(object):
    ...
    def writeToAerodynFileAdwimo(self, filename):
        """Write the airfoil section data to a file using AeroDyn input file style with Adwimo.
        ...
        f.write('AeroDyn airfoil file generated by airfoilprep.py' + '\n')
        f.write('Compatible with AeroDyn v13.0.' + '\n')
        f.write('%i\t' % len(af.polars) +
                'Number of airfoil tables in this file' + '\n')
        for p in af.polars:
            f.write('%f\t' % (p.Re / 1e6) +
            ...
            # f.write('{0:<10f}\t{1:40}'.format(
            #    param[0], 'Control setting' + '\n'))
            f.write('%f\t' % param[1] + 'Stall angle (deg)' + '\n')
            for _ in range(3):
                f.write(str(0) + '\t' + 'No longer used, enter zero' + '\n')

            f.write('%f\t' % param[2] +
                    'Zero Cn angle of attack (deg)' + '\n')
            f.write('%f\t' %
                    param[3] + 'Cn slope for zero lift (dimensionless)' + '\n')
            f.write(
                '%f\t' % param[4] + 'Cn extrapolated to value at positive stall angle of attack' + '\n')
            f.write(
                '%f\t' % param[5] + 'Cn at stall value for negative angle of attack' + '\n')
            f.write('%f\t' %
                    param[6] + 'Angle of attack for minimum CD (deg)' + '\n')
            f.write('%f\t' % param[7] + 'Minimum CD value' + '\n')
            for a, cl, cd, cm in zip(p.alpha, p.cl, p.cd, p.cm):
                f.write('{:<10f}\t{:<10f}\t{:<10f}\t{:<10f}'.format(
                    a, cl, cd, cm) + '\n')
            #f.write('EOT' + '\n')
